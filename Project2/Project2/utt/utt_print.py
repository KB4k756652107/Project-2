...
def print_turn(player):
    ...
    for i in range(7):
        print(f"{segments[index_p+i]}{segments[index_t+i]}")
    print(f"{" "*15}{segments[index_t+7]}", flush=True)
...
def print_board(game_board, current_board_num):
    ...
    def sub_board_line(board_num, y):
        ...
        if y%2 == 1:
            for i in range(3):
                text = f"{text} {board(board_num)[sub_row+i]} "
                if i//2 != 1:
                    text = text + "|"
        else:
            text = "-----------"
        ...
        for a in range(7):
            for b in range(3):
                if board_row[b] != " ":
                    ...
                    line_seg = sub_board_line(row+b+1, a)
                line_seg = f"{add_space(line_seg)}{line_seg}{add_space(line_seg)}"
                board_line = f"{board_line}{line_seg}"
                if b//2 != 1:
                    board_line = f"{board_line}|"
            print(f"{gap}{board_line}", flush=True)
            board_line = ""
            if a%2 == 1:
                sub_row += 3
        if row != 6:
            print(f"{gap}{segments[text_index["border"]]}", flush=True)
        sub_row = 0
...
def print_win_state(win_state, player):
    if win_state == True:
        ...
        gap = " "*6
        for i in range(7):
            print(f"{gap}{segments[index_p+i]}{segments[index_w+i]}")
        print(f"{gap}{" "*15}{segments[index_w+7]}", flush=True)
    else:
        index_d = text_index["draw"]
        gap = " "*14
        for i in range(7):
            print(f"{gap}{segments[index_d+i]}", flush=True)
        print('\n', flush=True)